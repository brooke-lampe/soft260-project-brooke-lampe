...
/* eslint-disable no-magic-numbers, no-underscore-dangle, no-unused-vars */
...
QUnit.test('Simulate a passenger to check duration (1)', (assert) => {
  const a = new Vertex('a');
  const b = new Vertex('b');
  const c = new Vertex('c');
  const d = new Vertex('d');
  ...
  const city = new City(graph, graph);
  const passenger = new Passenger(city, 'p', 4.0, a);
  passenger.source = a;
  passenger.destination = c;
  passenger.start();
...
QUnit.test('Simulate a passenger to check duration (2)', (assert) => {
  const a = new Vertex('a');
  const b = new Vertex('b');
  const c = new Vertex('c');
  const d = new Vertex('d');
...
QUnit.test('Simulate no passengers for findTopKDuration (1)', (assert) => {
  ...
  graph.addEdge(e, new UndirectedEdge(2), a);
  const city = new City(graph, graph);
  assert.deepEqual(city.findTopKDuration(5), []);
});

QUnit.test('Simulate one passenger for findTopKDuration (2)', (assert) => {
  const a = new Vertex('a');
  const b = new Vertex('b');
  const c = new Vertex('c');
  const d = new Vertex('d');
...
QUnit.test('simulate passengers alighting after the bus they are riding is stopped', (assert) => {
  ...
  time = city.step();
  assert.deepEqual(time, 2);
  assert.deepEqual(companion.bus, x);
  time = city.step();
  assert.deepEqual(time, 3);
  x.stop();
  time = city.step();
